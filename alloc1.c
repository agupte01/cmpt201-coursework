...
void *alloc(int size) {
  ...
  uint64_t total_size =
      ...
      (uint64_t)size; // align8_u64(sizeof(struct header) + (uint64_t)size);

  printf("alloc:(%d) -> total_size=%lu header=%lu\n", size,
         (unsigned long)total_size, (unsigned long)sizeof(struct header));
  if (total_size < sizeof(struct header) + sizeof(struct header *)) {
    total_size = sizeof(struct header) + sizeof(struct header *);
  }
  ...
  if (curr) {
    uint64_t remainder = curr->size - total_size;

    printf("found block:ccurr=%p curr->size%lu total_size=%lu remainder=%lu\n",
           (void *)curr, (unsigned long)curr->size, (unsigned long)total_size,
           (unsigned long)remainder);
    if (remainder > sizeof(struct header) + 1) {
      struct header *new_block = (struct header *)((char *)curr + total_size);
      new_block->size = remainder;
      new_block->next = curr->next;

      curr->size = total_size;
      printf("split allocated block at %p size=%lu, new free block at %p "
             "size=%lu\n",
             (void *)curr, (unsigned long)curr->size, (void *)new_block,
      ...
      printf("no split: using entire block at %p size=%lu (remainder=%lu too "
             "small)\n",
             (void *)curr, (unsigned long)curr->size, (unsigned long)remainder);
      if (prev)
        prev->next = curr->next;
      else
        free_list = curr->next;
    }
    void *ret = (void *)((char *)curr + sizeof(struct header));
    printf("returning pointer: %p (block at %p)\n\n", ret, (void *)curr);
    return ret;
    //////return (void *)((char *)curr + sizeof(struct header));
  }
  /*
if (curr) {
  uint64_t need = sizeof(struct header) + (uint64_t)size;
...
*/
  if (heap_limit == 0 || current_heap_size + INCREMENT <= heap_limit) {
    ...
    // new_header->size = (uint64_t)INCREMENT; //
    // align8_u64((uint64_t)INCREMENT); printf("arena grow: new_header=%p
    // new_header->size=%llu\n",
    ...
    struct header *q = free_list;
    while (q && q < new_header) {
      p = q;
      q = q->next;
    }
    ...
    while (scan && scan->next) {
      if ((char *)scan + scan->size == (char *)scan->next) {
